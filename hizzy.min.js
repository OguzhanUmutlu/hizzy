...
module.exports = class MyAddon extends AddonModule {
  ...
  "version": "1.0.0",
  "main": "index.js"
}`),printer.raw.pass("A new addon has been set up at: %c"+t,"color: orange"),process.exit()}if(_argv_.build||_argv_["no-clear"]||(printer.clear(),printer.print("\n")),isTerminal&&args[0]){const v=path.join(process.cwd(),args[0]);fs.existsSync(v)||exit("Couldn't open the file %c'"+v+"'%c.","color: orange","color: red"),dir=fs.statSync(v).isDirectory()?v:(file=v,path.dirname(v))}(async()=>{var e={dev:!0,port:-1,fileRefresh:!0,autoBuild:!0,listen:!0,main:"Server.jsx",mainModule:!0,checkConfig:!0,realtime:!0,static:{},https:!1,srcFolder:"src",connectionTimeout:6e4,keepaliveTimeout:3e4,clientKeepalive:2e4,minClientKeepalive:8e3,addons:[],includeOriginalInBuild:!0};let r=_argv_.config;if(r&&!fs.existsSync(path.join(dir,r)))return exit("Config file %c"+r+"&t given in the command line arguments is invalid:","color: orange");if(!r)for(const c of["json","config.json","config.ts","config.js","config.mts","config.mjs"])if(r=__PRODUCT__+"."+c,fs.existsSync(path.join(dir,r)))break;var o=path.join(dir,r),t=fs.existsSync(o);t&&fs.statSync(o).isFile()||(t&&fs.rmSync(o),_argv_.debug&&printer.dev.debug("Creating the %c/"+r+"&t file...","color: orange"),fs.writeFileSync(o,`import {defineConfig} from "hizzy";
